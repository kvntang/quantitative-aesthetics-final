...
while cap.isOpened():
    ...
    x, y, r, activation = grid_search(img, 1, goal_class_index, 4, radii)

    print(f'General Looking result: {x}, {y}, {r}, {activation}')
    ...
    for i in range(50):
        gradient_descent(img, P, ds, gradient_step_size)

        # Display xyr with new gradient
        print(f'Precise Looking result #{i}: {P}')
        # spotlight_mask(img, image_size, P)
        if (P[2] < 1):
            P[2] = 1
        ...
        cv2.putText(image_w_mask, f'looping: {i}', (10,200), cv2.FONT_HERSHEY_PLAIN,
                1, (255, 255, 255), 1)
