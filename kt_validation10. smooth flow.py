...
def clamp_params():
    ...
    if P[0]<border:
        P[0] = border
    elif P[0]>image_size-border:
        P[0] = image_size - border

    if P[1]<border:
        P[1] = border
    elif P[1]>image_size-border:
        P[1] = image_size - border
...
# cv2.createTrackbar('Radius', 'Smooth Movement', 0, 50, update_radius_index)
    ...
    if current_mode == AMODES.GRID:    
        ...
        for j in range(rows):
            ...
            for i in range(cols):
                ...
                sub_img = frame[fj:fj+ch, fi:fi+cw, :]
                sub_perc, sub_act  = validate_image(sub_img, goal_class_index, device, classifierModel)
                sub_perc *= 100.0
                frame_clone = cv2.putText(frame_clone, f"{sub_perc:.1f}%", (fi + cw//2, fj + ch//2), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 1)

        cv2.imshow('Smooth Movement', frame_clone)


    elif current_mode == AMODES.TRACK:

        img = cv2.resize(frame, (image_size, image_size))
        img = img.astype(np.float32) / 255.0

        for k in range(2):
            gradient_descent(img, P, [random.uniform(20.0, 50.0), random.uniform(20.0, 50.0), 0.0], -455.0)
            clamp_params()
            px = int(P[0] * frame_w/image_size)
            py = int(P[1] * frame_h/image_size)
            rr = int(P[2] * frame_h/image_size)

            frame = cv2.circle(frame, (px,py), rr, (255,0,0), 3, cv2.LINE_AA)

        cv2.imshow('Smooth Movement', frame)
