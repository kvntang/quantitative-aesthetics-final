...
def update_class_index():
    global goal_class_index
    goal_class_index = class_index_var.get()

def update_camera():
    global goal_class_index, x, y, r
    while True:
        ...
        P = [x, y, r]  # circle parameters
        ds = [20.0, 20.0, 2.0]  # step distance
        iteration_count = 100
        gradient_step_size = 20

        for i in range(10):
            # ... Your existing OpenCV processing code ...
            # gradient_descent_with_randomness

            # Check activation and adjust P
...
# Toggle button for class index
class_index_var = tk.IntVar(value=goal_class_index)
class_toggle = tk.Checkbutton(root, text="Toggle Class Index", variable=class_index_var, command=update_class_index)
class_toggle.pack()

# Start the OpenCV camera feed in a separate thread
